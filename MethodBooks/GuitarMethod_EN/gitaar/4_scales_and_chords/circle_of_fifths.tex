...
But if you look at the scale of E minor (in \autoref{tab:guitar_minor_scales_circle_of_fifths}) then you see that G is the 3rd degree in the scale (a minor 3rd interval). Meaning that the G is an important note that makes the E minor scale, minor.
...
\autoref{fig:circle_of_fith_overlap_c_minor_f_major} shows that there are two overlapping chords (B$\flat$, G minor) and five overlapping notes (B$\flat$, C, D, F, G) in these two scales (In the C minor scale the \textit{d} is a diminished chord, while in F major \textit{d} is a minor chord). In "We Are The Champions" the B$\flat$ is used as the pivot chord/note to connect the C minor to the F major.
...
\begin{song}[align-chords=l]{title={We Are The Champions - Queen}, music={Queen}}
	...
	\begin{chorus}
		...
		^{Gm7}We are the ^{C7/G}champions, ^{Bbm6}we are the ^{Edim7}cham - ^{Gdim7}pions \\
		^{F}No time for ^{Ebadd9/G}losers 'cause ^{Ab6}we are the ^{Bb}champions ^{C7sus4} \\
		...
 	\end{chorus}
\end{song}
