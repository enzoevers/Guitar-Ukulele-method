...
Now when you create a chord progression for a mode, it would be good to use at least one chord listed in \autoref{tab:guitar_mode_characteristic_chords} besides having the first chord of the mode. Hearing the differences between modes is generally a lot harder than hearing the differences between major and minor.

Therefore, \textbf{keep it simple}. Modal chord progressions should emphasize the characteristic notes and the root note of the mode. There is less need to 'resolve' the modal chord progressions. Using the characteristic notes in the melody that is played over the chord progressions helps as well.

A practical exercise to hear how the different modes 'feel' is to have a drone note of the root of your mode and then play the mode over it. A drone note is a note that is continuously playing in the background. In \autoref{fig:guitar_diatonic_modes_on_guitar_with_drone_note_single_string} the drone note is the open low E string. Then the different modes can be played on the 4th string. By making melodies even on a single string while having the open E string as the drone note can give some nice sounding results.
...
\begin{figure}[h]
	...
	\caption{Diatonic modes on a single string on the guitar with a drone note}
...
\autoref{fig:guitar_mode_scale_shapes} and \autoref{fig:guitar_diatonic_modes_on_guitar} show the different mode-shapes. In \autoref{fig:guitar_diatonic_modes_on_guitar} we start with F$\sharp$ Ionian (major) scale. But remember that these can be shifted up or down to be in a different key.

The gray frets with numbers on the 6th string in \autoref{fig:guitar_diatonic_modes_on_guitar} show the F$\sharp$ major scale on the 6th string. This is used to show the connection between the starting position of the different modes with the major scale intervals. Because we are looking from the major scale's perspective, the numbers in the frets of a shape also correspond to the intervals in the major scale.
